...
random input obtained from polling various entropy sources. The
exact features used depend on how OpenSSL was configured, and a summary
can be displayed with the OpenSSL L<version(1)> command. If the default 
CSPRNG is the new OpenSSL CTR_DRBG, then calling this function is not 
necessary, because DRBGs manage their seeding during instantiation and 
reseeding automatically. But calling RAND_poll() is allowed also in this
case and  results in a reseeding of the default CTR_DRBG.
