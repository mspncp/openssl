...
static int drbg_hash_instantiate_wrapper(void *vdrbg, int strength,
                                         ...
                                         size_t pstr_len)
{
    ...
    return PROV_DRBG_instantiate(drbg, strength, prediction_resistance,
                                 pstr, pstr_len, &drbg_hash_instantiate);
}
                                    ...
                                    const unsigned char *adin, size_t adin_len)
{
    ...
    return PROV_DRBG_reseed(drbg, prediction_resistance, ent, ent_len,
                            adin, adin_len, &drbg_hash_reseed);
}
     ...
     const unsigned char *adin, size_t adin_len)
{
    ...
    return PROV_DRBG_generate(drbg, out, outlen, strength,
                              ...
                              &drbg_hash_generate, &drbg_hash_reseed);
}
...
static int drbg_hash_new(PROV_DRBG *ctx, int secure)
{
    PROV_DRBG_HASH *hash;
...
static void *drbg_hash_new_wrapper(void *provctx, int secure, void *parent,
...
static int drbg_hash_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    ...
    if (!ossl_prov_digest_load_from_params(&hash->digest, params, ctx->libctx))
        return 0;
