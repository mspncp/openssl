...
static int drbg_ctr_instantiate_wrapper(void *vdrbg, int strength,
                                        ...
                                        size_t pstr_len)
{
    ...
    return PROV_DRBG_instantiate(drbg, strength, prediction_resistance,
                                 pstr, pstr_len, &drbg_ctr_instantiate);
}
     ...
     const unsigned char *adin, size_t adin_len)
{
    ...
    return PROV_DRBG_generate(drbg, out, outlen, strength,
                              ...
                              &drbg_ctr_generate, &drbg_ctr_reseed);
}
...
static int drbg_ctr_new(PROV_DRBG *drbg, int secure)
{
    PROV_DRBG_CTR *ctr;

    ctr = drbg->secure ? OPENSSL_secure_zalloc(sizeof(*ctr))
...
static void *drbg_ctr_new_wrapper(void *provctx, int secure, void *parent,
...
static int drbg_ctr_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    ...
    if ((p = OSSL_PARAM_locate_const(params, OSSL_RAND_PARAM_USE_DF)) != NULL
    ...
    if ((p = OSSL_PARAM_locate_const(params,
                                     OSSL_RAND_PARAM_PROPERTIES)) != NULL) {
        if (p->data_type != OSSL_PARAM_UTF8_STRING)
            return 0;
        propquery = (const char *)p->data;
    }

    if ((p = OSSL_PARAM_locate_const(params, OSSL_RAND_PARAM_CIPHER)) != NULL) {
        if (p->data_type != OSSL_PARAM_UTF8_STRING
                || p->data_size < 3
                || (ecb = strdup((char *)p->data)) == NULL)
            return 0;
        strcpy(ecb + p->data_size - (sizeof("ECB") - 1), "ECB");
        EVP_CIPHER_free(ctr->cipher_ecb);
        EVP_CIPHER_free(ctr->cipher_ctr);
        ctr->cipher_ctr = EVP_CIPHER_fetch(ctx->libctx, p->data, propquery);
        ctr->cipher_ecb = EVP_CIPHER_fetch(ctx->libctx, ecb, propquery);
        OPENSSL_free(ecb);
