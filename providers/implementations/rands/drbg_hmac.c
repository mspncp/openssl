...
static int drbg_hmac_instantiate_wrapper(void *vdrbg, int strength,
                                         ...
                                         size_t pstr_len)
{
    ...
    return PROV_DRBG_instantiate(drbg, strength, prediction_resistance,
                                 pstr, pstr_len, &drbg_hmac_instantiate);
}
                                    ...
                                    const unsigned char *adin, size_t adin_len)
{
    ...
    return PROV_DRBG_reseed(drbg, prediction_resistance, ent, ent_len,
                            adin, adin_len, &drbg_hmac_reseed);
}
     ...
     const unsigned char *adin, size_t adin_len)
{
    ...
    return PROV_DRBG_generate(drbg, out, outlen, strength,
                              ...
                              &drbg_hmac_generate, &drbg_hmac_reseed);
}
...
static int drbg_hmac_new(PROV_DRBG *drbg, int secure)
{
    PROV_DRBG_HMAC *hmac;

    hmac = drbg->secure ? OPENSSL_secure_zalloc(sizeof(*hmac))
...
static void *drbg_hmac_new_wrapper(void *provctx, int secure, void *parent,
...
static int drbg_hmac_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    ...
    if (!ossl_prov_digest_load_from_params(&hmac->digest, params, ctx->libctx))
        return 0;
...
static const OSSL_PARAM *drbg_hmac_settable_ctx_params(void)
{
    static const OSSL_PARAM known_settable_ctx_params[] = {
        ...
        OSSL_PARAM_utf8_string(OSSL_RAND_PARAM_MAC, NULL, 0),
