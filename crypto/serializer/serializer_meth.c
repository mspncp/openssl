...
static void *serializer_store_new(OSSL_LIB_CTX *ctx)
{
    return ossl_method_store_new(ctx);
}


static const OSSL_LIB_CTX_METHOD serializer_store_method = {
...
static void *alloc_tmp_serializer_store(OSSL_LIB_CTX *ctx)
{
    return ossl_method_store_new(ctx);
}
...
static void *get_serializer_from_store(OSSL_LIB_CTX *libctx, void *store,
                                       void *data)
{
...
static int put_serializer_in_store(OSSL_LIB_CTX *libctx, void *store,
...
static OSSL_SERIALIZER *inner_ossl_serializer_fetch(OSSL_LIB_CTX *libctx,
...
OSSL_SERIALIZER *OSSL_SERIALIZER_fetch(OSSL_LIB_CTX *libctx, const char *name,
                                       const char *properties)
{
    return inner_ossl_serializer_fetch(libctx, 0, name, properties);
}

OSSL_SERIALIZER *ossl_serializer_fetch_by_number(OSSL_LIB_CTX *libctx, int id,
                                                 const char *properties)
{
...
int OSSL_SERIALIZER_is_a(const OSSL_SERIALIZER *ser, const char *name)
{
    if (ser->base.prov != NULL) {
        OSSL_LIB_CTX *libctx = ossl_provider_library_context(ser->base.prov);
        OSSL_NAMEMAP *namemap = ossl_namemap_stored(libctx);
...
void OSSL_SERIALIZER_do_all_provided(OSSL_LIB_CTX *libctx,
